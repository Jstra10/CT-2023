...
```{r}
# filter info in the DF, where the month is equal to 3 and day = 31

# filterdf,conditional 1, conditional 2
...
```{r}
delayed_times_df <- filter(flights, dep_time <= 1000 & sched_arr_time != 0)
...
# select colums that start with the letter c
...
```{r}
# Select all columns from the flight
...
select(flights, ends_with('g'))



#view(select(flights, matches('mp|c')))
View(mtcars)

mtcars_mut <- mutate(mtcars,new_column = disp * cyl)
                          
view(mtcars_mut)


#summarize(flights, delay= mean(dep_delay, na.rm=TRUE))
by_cyl <- group_by(cyl,mpg,hp)
summarize(by_cyl, avgs = mean(gear), na.rm=TRUE)
...
own_column <-  flights %>%
  ...
  summarize(mean = mean(sched_arr_time),)
view(own_column)
...
```{r}
ggplot(data=my_plot,mapping = aes(disp, hwy, color = displ))
geom_point()
```
...
```{r}
ggplot(data=my_plot) + 
  geom_point(mapping = aes(displ, hwy), postition = 'jitter')
```
...
geom_bar(aes(cut, fill=cut), position = 'identity')
```
...
```{r}
out_function <- function(optional_params){
  # do stuff
  # return stuff
}
# to invoke:
...
```{r}
test_temps <- c(16.55,26.99,32.59,8.58)
...
1.  Do 5 CodeWars problems in R. Share your solutions here:
    ...
    # Question 5: Beginner Series #1 School Paperwork
...
-   Column 1: Data Type: Continuous Data

-   Column 2: Data Type: Discrete Data
...
```{r}
select(USArrests, everything())
head(USArrests)
#select(USArrests, Assault)

df <- mutate(USArrests,
                          total_assaults = Assault * 100000)

View(df)

group1 <- group_by(df, Assault, total_assaults,Murder,UrbanPop)
View(summarize(group1, assault_mean=mean(total_assaults, na.rm=TRUE)))
```

```{r}
qplot(Assault, UrbanPop, data=df,
      colour = df[[1]],
      size=1,
      alpha = 1/100)
...
```{r}
qplot(Murder, UrbanPop, data=df)
```

```{r}
qplot(UrbanPop, Murder*Assault*Rape, data=df)


```

```{r}
ggplot(data=df,mapping = aes(Murder, Assault, color = df[[1]])) +
  geom_point()
```
...
```{r}
# Hypotheses: As Urban Population increases, arrests increase
```
